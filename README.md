...
**本ソフトウェアはオープンソースソフトウェアではありません。** 著作権は Small Works Lab が保持します。

ソースを公開しているのは、**このツールが何を読み、どこへ送り、何をしないのかを
利用者自身が確認できるようにするため**であり、再配布や再販を許諾するものではありません。

- ✅ 閲覧・監査・セキュリティ検証
- ✅ **自分自身が使う目的でのビルドと利用**
- ❌ 再配布（無償・有償を問わない）、販売、商用製品への組み込み

詳細は [LICENSE.md](LICENSE.md) を参照してください。

ビルド済みの実行ファイルは [BOOTH](https://smallworkslab.booth.pm/items/8444958) で配布しています（300円）。
中身は同じものです。Python の環境構築をせずにそのまま使いたい方向けです。
